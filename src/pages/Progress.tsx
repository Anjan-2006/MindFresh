...
import { BarChart3, TrendingUp, Calendar, Heart, Brain, Target } from "lucide-react";
import {
  LineChart,
  Line,
  BarChart,
    ...
    try {
      ...
      if (weeklyMoods) {
        ...
        const chartData = Object.values(moodsByDay).map((day: any) => ({
          ...day,
          mood: day.moods.reduce((sum: number, mood: number) => sum + mood, 0) / day.moods.length
        }));
      ...
      if (monthlyMoods && monthlyMoods.length > 0) {
        const avg = monthlyMoods.reduce((sum, entry) => sum + entry.mood_value, 0) / monthlyMoods.length;
      ...
      const meditationScore = Math.min(((monthlyMeditation?.reduce((sum, session) => sum + session.session_duration, 0) || 0) / 60) * 2, 20);
      ...
      const generatedInsights = [
        {
          icon: TrendingUp,
          title: "Mood Trend",
          description: monthlyMoods && monthlyMoods.length > 0 
            ? `Your average mood this month is ${((monthlyMoods.reduce((sum, entry) => sum + entry.mood_value, 0) / monthlyMoods.length)).toFixed(1)}/10`
        ...
        },
        {
          ...
          description: `You've written ${monthlyJournals?.length || 0} journal entries this month`,
          status: "positive",
        },
        {
          icon: Target,
          title: "Meditation Progress",
          description: `${Math.floor(((monthlyMeditation?.reduce((sum, session) => sum + session.session_duration, 0) || 0) / 60))} minutes of meditation this month`,
          status: "neutral",
        },
      ];
      ...
      const generatedAchievements = [
        { 
          ...
          earned: (monthlyJournals && monthlyJournals.length > 0) || (monthlyMoods && monthlyMoods.length > 0)
        },
        { 
          title: "Journal Habit", 
        ...
        { 
          ...
          earned: ((monthlyMeditation?.reduce((sum, session) => sum + session.session_duration, 0) || 0) / 60) >= 30
        },
        { 
          title: "Mood Tracker", 
      ...
      for (let i = 3; i >= 0; i--) {
        ...
        stats.push({
          ...
          moodAvg: weekMoods && weekMoods.length > 0 
            ? Number((weekMoods.reduce((sum, entry) => sum + entry.mood_value, 0) / weekMoods.length).toFixed(1))
            : 0,
          meditation: Math.floor((weekMeditation?.reduce((sum, session) => sum + session.session_duration, 0) || 0) / 60)
        });
      }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-3xl font-bold text-wellness-green">{averageMood || '0'}</p>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-3xl font-bold text-wellness-blue">{journalEntries}</p>
                  <p className="text-sm">This month</p>
                </div>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-3xl font-bold text-wellness-purple">{wellnessScore}</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          ...
          <Card className="wellness-card">
            <CardHeader>
              ...
              <CardDescription>How you've been feeling this month</CardDescription>
            </CardHeader>
            <CardContent>
              <ResponsiveContainer width="100%" height={300}>
                <PieChart>
        ...
        <Card className="wellness-card mb-8">
          <CardHeader>
            ...
            <CardDescription>Your wellness activities across the past 4 weeks</CardDescription>
          </CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={weeklyStats}>
                ...
                <YAxis />
                <Tooltip />
                <Bar dataKey="journalEntries" fill="hsl(142 76% 48%)" name="Journal Entries" />
                <Bar dataKey="meditation" fill="hsl(200 98% 65%)" name="Meditation (minutes)" />
              </BarChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <Card className="wellness-card">
            <CardHeader>
              ...
              <CardDescription>Personalized observations about your wellness journey</CardDescription>
            </CardHeader>
            <CardContent>
                ...
                {insights.map((insight, index) => (
                  <div key={index} className="flex items-start space-x-3 p-3 rounded-lg bg-muted/50">
                    <insight.icon className={`h-5 w-5 mt-0.5 ${
                      insight.status === 'positive' ? 'text-wellness-green' : 'text-wellness-blue'
                    }`} />
                    <div>
              ...
              <div className="mt-6 p-4 bg-gradient-secondary rounded-lg">
                ...
                <p className="text-sm text-muted-foreground">
                  Your mood tends to be highest on Fridays. Try scheduling important conversations 
                  or activities on these days to maximize your positive energy.
                </p>
              </div>
            </CardContent>
          </Card>
