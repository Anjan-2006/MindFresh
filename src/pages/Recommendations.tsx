    ...
    try {
      const response = await fetch(
        `https://discoveryprovider.audius.co/v1/tracks/search?query=${encodeURIComponent(
          query
        )}&limit=20&app_name=mindfresh`
      );

      if (response.ok) {
        const data = await response.json();
        // Filter out short tracks (< 30 seconds) to get full songs only
        const fullTracks = (data.data || []).filter(
          (track: Track) => track.duration > 30
        );
        ...
        <div className="mb-8 text-center">
          ...
          <div className="flex items-center justify-center space-x-2 mb-4">
            ...
            <p className="text-lg text-muted-foreground">
              Based on your current mood ({currentMood}/10), here are some{" "}
              {getMoodDescription(currentMood)} recommendations
            </p>
          </div>
          <Button
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {tracks.length === 0 && !loading ? (
              <p className="text-muted-foreground col-span-full text-center py-8">
                No music recommendations found. Try refreshing or check your
                connection.
              </p>
            ) : (
              tracks.map((track) => (
        ...
        <section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {books.map((book) => (
              <Card
                key={book.id}
                className="wellness-card cursor-pointer hover:shadow-lg transition-shadow"
